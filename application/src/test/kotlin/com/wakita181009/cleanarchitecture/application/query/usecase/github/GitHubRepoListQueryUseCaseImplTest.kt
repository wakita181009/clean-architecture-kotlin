...
class GitHubRepoListQueryUseCaseImplTest {
    ...
    fun `execute returns Right with page when parameters are valid`() {
        ...
        every { queryRepository.list(offset = 0, limit = 20) } returns Either.Right(page)
    ...
    fun `execute returns Right with empty page`() {
        ...
        every { queryRepository.list(offset = 0, limit = 20) } returns Either.Right(emptyPage)
    ...
    fun `execute returns Left FetchFailed when repository returns error`() {
        ...
        every { queryRepository.list(offset = 0, limit = 20) } returns Either.Left(fetchError)
    ...
    fun `execute calculates correct offset for page 2`() {
        ...
        every { queryRepository.list(offset = 20, limit = 20) } returns Either.Right(page)
