    ...
    fun `of returns Right with correct value for any non-blank string`() =
        runTest {
            checkAll(Arb.string(1..100).filter { it.isNotBlank() }) { s ->
                GitHubRepoName.of(s).shouldBeRight().value shouldBe s
            }
        }

    @Test
    fun `of returns Left InvalidName for blank strings including unicode whitespace`() =
        runTest {
            checkAll(Arb.of("", " ", "   ", "\t", "\n", "\r\n", "\u00A0")) { s ->
                GitHubRepoName.of(s).shouldBeLeft().shouldBeInstanceOf<GitHubError.InvalidName>()
            }
        }
